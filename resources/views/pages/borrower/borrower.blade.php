...
@section('content')
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-600 mt-2">Organize and manage book borrowers for your library collection</p>
        </div>
        
        <!-- Create Category Button -->
        <a href="{{ route('borrowers.create') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200">
    ...
    <div class="bg-white/80 backdrop-blur-sm rounded-xl shadow-lg border border-amber-100 overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-amber-100 bg-gradient-to-r from-amber-50 to-orange-50">
            <div class="flex items-center space-x-2">
                ...
                <h3 class="text-lg font-semibold text-gray-900">All Borrowers</h3>
            </div>
        </div>
                    ...
                    @forelse($borrowers as $data)
                        <tr class="hover:bg-amber-50/50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600 max-w-xs">
                                <p class="truncate">{{ $data->email ?? 'No email' }}</p>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                {{ $data->phone ?? 'No phone' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                {{ $data->institution ?? 'No institution' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                <div class="flex items-center justify-center space-x-2">
                                    <!-- Edit Button -->
                                    <a href="{{ route('borrowers.edit', $data->id) }}" 
                                    ...
                                    <!-- Delete Button -->
                                    <form method="POST" action="{{ route('borrowers.destroy', $data->id) }}" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
                                                onclick="return confirm('Are you sure you want to delete this borrower? This action cannot be undone.')"
                            ...
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-gray-600 mb-6 max-w-md">
                                        Get started by creating your first book borrower. Borrowers help organize your library collection efficiently.
                                    </p>
        ...
        <div class="px-6 py-3 bg-gray-50/80 border-t border-gray-200 flex items-center justify-between">
            <div class="text-sm text-gray-700">
                Showing <span class="font-medium">{{ $borrowers->count() }}</span> borrowers
            </div>
